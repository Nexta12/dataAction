...
import { Consultations } from "@customTypes/consultationTypes";
...
const ConsultationTable = () => {
  ...
  const confirmDelete = async () => {
    if (itemToDelete) {
      try {
        ...
        await apiClient.delete(`${endpoints.deleteConsultationRequest}/${itemToDelete}`);
  ...
  const keyExtractor = (row: Consultations) => row._id;
    ...
    <div className="">
      ...
      <div className=" flex  items-center justify-between mb-8">
        <div className="flex-1">
          <SubHeading className="">Consultation Requests({data.length})</SubHeading>
        </div>
        <div className="flex-1">
