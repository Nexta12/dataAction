...
import { FaList, FaObjectGroup, FaQuestion, FaThList } from "react-icons/fa";
    ...
    <section className="w-full m-0 p-0 bg-admingray relative">
      ...
      <div className="header bg-admingray p-5 md:p-10  xl:px-3 mb-6 shadow flex gap-6 items-center justify-between sticky top-0 z-50 ">
        ...
        <div className="left flex items-center  gap-4 md:gap-12">
            {!sidepanel ? ( <LuMenu className=" md:hidden text-xl"  onClick={handleSidepanel} /> ) : <IoMdClose className="md:hidden text-xl rounded-full border border-danger"  onClick={handleSidepanel} />}
      ...
      <div className="mainbody flex items-start gap-8">
        ...
        <div className="sidebar hidden md:block flex-1 shadow bg-white border-r-2 border-LightBlue xl:pl-3 pt-4 h-[80vh] text-md text-dark">
          <ul className="m-0 p-0 flex flex-col gap-2">
            ...
            <li key={index} className="flex items-center gap-3 cursor-pointer  p-3  hover:bg-dark hover:text-white">
        ...
        <div className="conntentBody flex-[5] w-full px-4">
           {children}
        </div>
      </div>

       {/* Sidepanel */}
      <div className={`drawer h-[90vh] bg-white transition-all duration-300 absolute top-[80px] md:hidden w-full ${!sidepanel && "-translate-x-full xl:translate-x-0"} `}>
        <ul className="m-0 pl-3 flex flex-col gap-2 w-full text-md text-dark pt-6">

            { DashBoardMenu.map((item, index)=>(
          <li key={index} className="flex items-center gap-3 cursor-pointer  p-3  hover:bg-dark hover:text-white" onClick={handleSidepanel}>
            <item.icon className="" /> <a href={item.link}>{item.title}</a>
          </li>
           ))}
